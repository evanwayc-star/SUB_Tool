...
播放期間動畫迴圈以 `requestAnimationFrame` 同步 `tcCur` 與 `seekBar`，不依賴較低頻的瀏覽器 `timeupdate`；打點 In／Out 於按鍵事件觸發時捕獲 `Media.displayTime()`，避免非同步作業或彈窗延遲造成時碼漂移。預覽 Proxy 的短 GOP 可減少解碼等待，但不改變本節的時間權威：只有實際呈現的畫格能提交呈現位置。
