...
> 對應版本：v5.1.0｜主題：影格率（FPS）與時碼（timecode）在各處顯示為何要一致、如何保證一致。
...
- 網頁版：`detectFpsWeb()`（`media.js`）用 `requestVideoFrameCallback` 量真實影格時間戳的中位數。**量到的值不可先 `Math.round()` 取整**——整數化會把 29.97→30、23.976→24，直接把 NTSC 分數影格率毀掉（v5.1.0 修復）；原始實測值直接交給 `snapFps()`。
...
- 24 與 23.976 只差 **0.024**、30 與 29.97 只差 0.03。容差一旦 ≥ 0.024，整數影格率就會被誤判成 NTSC 分數影格率——v5.1.0 曾把容差放寬到 0.05，造成**所有 24/30fps 專案時碼每小時漂移約 3.6 秒**（v5.1.0 修復，容差定為 **0.01**）。
