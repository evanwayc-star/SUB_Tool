...
`buildDeliveryArgv()` 從完整工作建立交付計畫，自行套用固定規格、正規化音訊並計算有效片長，再決定航空 codec／mux 或光碟容量預算。runner 不傳入預先算好的格式設定，避免較長音訊尾端與容量估算、DMPES 4M 與 1.5M transport profile 各自配錯。`airline-encoding.js` 與 `disc-encoding.js` 是其內部純規格實作；輸入能力驗證仍由主程序執行。
