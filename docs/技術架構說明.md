...
- `electron/media-intake-runtime.js`：probe、Proxy、波形、聲道快取與 lease。預覽 Proxy 全面採用 720p 全 I 幀（All-Intra / GOP = 1、無 B 幀、閉合 GOP）規格，消除幀間相依性，確保隨機 seek、倒帶與逐格反應時間為 0ms 且完全不抖動。
- `electron/mpv-host.js`：Windows mpv 子視窗與精準呈現。
- `electron/export-queue.js`／`delivery-runner.js`：背景交付。遵循標準目標位元率與長 GOP 壓縮標準，嚴守鐵律 §0.8（匯出只讀母素材，絕不碰 Proxy 快取）。
...
| 媒體准入 | `media-intake-engine.js`、`loaders/media-loader.js` | 單次載入 identity、取消與提交 |
