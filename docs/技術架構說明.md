...
| `decode/player.js` | ~310 | **WebCodecs 預覽引擎**：多軌即時合成（見 §8） |
| `notes.js` | ~300 | 備註（時間點標記，匯出 CSV / EDIUS） |
| `settings.js` | ~295 | 設定對話框 |
| `substyle.js` | ~273 | **字幕樣式核心**：`effStyle` / CSS 與 ASS 轉換 / 字型 / 常用樣式庫（見 §7） |
...
| `decode/demux.js` | ~203 | **串流式 demux**：只讀檔頭建索引、位元組按需抓（見 §9） |
...
## 8. WebCodecs 預覽引擎（v4.20 起；`decode/player.js`）
...
每個來源檔一個 SourceStream
  demux（索引，見 §9）→ VideoDecoder → frames[] 淺佇列 → 取最接近 t 的 frame
rafLoop 每幀 tick()：
...
## 9. 串流式 demux（v4.29 起；`decode/demux.js`）
