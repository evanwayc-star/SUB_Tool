...
const spec = format => ({ format, clips: [source], width: 1920, height: 1080,
  fps: 25, videoKbps: 5000, duration: 3, outPath: `video${airlineElementaryEncoding(format).videoExtension}` });

describe('航空分流交付計畫', () => {
  it.each(['airline-s3k', 'airline-dmpes'])('%s 固定 progressive 並在顯示比例燒錄後轉非方形像素', format => {
    const encoding = airlineElementaryEncoding(format);
    const audioOutPath = `audio${encoding.audioExtension}`;
    ...
    }, { elementaryEncoding: encoding, audioOutPath, hasAudioStream: () => false, timecodeFontFile: 'font.ttf',
    ...
    expect(plan.args.at(-1)).toBe(audioOutPath);
    expect(plan.args.indexOf(spec(format).outPath)).toBeLessThan(plan.args.indexOf('-c:a'));
    expect(plan).toMatchObject({ isGpu: false, duration: 3, kbps: 1500, audioBitrates: ['128k'] });
  });

  it('缺少分流 adapter 不回退一般 H.264，也不偷偷丟棄多聲道', () => {
    expect(() => buildDeliveryArgv(spec('airline-s3k'))).toThrow('缺少影音分流');
