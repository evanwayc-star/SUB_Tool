/* ============================================================================
   TimelineTransport — 播放時間的純狀態核心

   對外公開的是時間軸時間；HTML video / mpv 回報的時間仍是來源時間。
   Media 只負責讀寫 driver，這個模組負責：
   - source ↔ timeline 的明確邊界
   - 唯一的 seek / pause 影格吸附
   - 無影片與純音訊的虛擬時鐘
   - 影片間隙的黑畫面時鐘
   - 暫停時的權威顯示位置
...
class TimelineTransport {
  ...
  /* sourceTime 僅從 Media driver 讀入；轉換函式由 sequence 層注入。 */
  ...
  startVirtual(time = this.virtualTimeAt(), { playbackRate = 1 } = {}){
    ...
    // Keep the argument explicit: callers use it to document the driver rate boundary.
    void playbackRate;
  ...
  pause({ sourceTime = 0, clip = null, virtual = false, playbackRate = 1, useGap = true, fps, dropFrame = false } = {}){
    ...
    if(virtual){
      // virtual clock 儲存的是 driver/source domain；paused 已是公開的 timeline
      // domain，帶 clip 時必須反向映射，否則下一次 display 會再加一次 offset/in。
