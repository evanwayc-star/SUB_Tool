...
`;
    ...
    if (!tracks || !tracks.length) {
      styles += `Style: Default,${defFont},48,&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100.0,100.0,0.0,0.0,1,3,0,2,135,135,108,1\n`;
    } else {
      tracks.forEach((tk, i) => {
        const fs = Math.round(48 * (tk.fontScale || 1));
        const pp = tk.posPct != null ? tk.posPct : 10;
        const mv = Math.round(1080 * (pp / 100));
        const col = hexToAss(tk.color || '#ffffff');
        styles += `Style: Track${i},${defFont},${fs},${col},&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100.0,100.0,0.0,0.0,1,3,0,2,135,135,${mv},1\n`;
      });
      styles += `Style: Default,${defFont},48,&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,0,0,0,0,100.0,100.0,0.0,0.0,1,3,0,2,135,135,108,1\n`;
    }
