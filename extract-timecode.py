...
app_js = re.sub(block, "", app_js, flags=re.DOTALL)

block2 = r"function renderTimecodeWatermark\(time=Media\.displayTime\(\)\)\{.*?(?=function toggleTimecodeWatermark)"
...
app_js = app_js.replace('renderTimecodeWatermark()', 'renderTimecodeWatermark(_videoWrap)')

open('src/app.js', 'w', encoding='utf-8').write(app_js)

print("Timecode watermark extracted")
